...
SHP_NAME = 'louisville_railroads_1957'
OUTPUT_NAME = 'louisville_railroads_1957_aligned'
shp_path = os.path.join(MAP_DIR, SHP_NAME)
...
dis_interpolate = 30
win_size = 30
color_low_bound = [100,65,70]#[120,80,90] (1957)
color_high_bound = [140,115,125]#[120,80,90]
...
for root, dirs, files in os.walk(shp_path):
    for f in files:
        if not os.path.exists(output_path+'\\'+f[:-4]+"_align.txt"):
            inputfile = open(root+'/'+f, 'r')
            outputfile = open(output_path+'\\'+f[:-4]+"_align.txt", 'w')
