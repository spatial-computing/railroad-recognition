...
def build_graph(root, debug=False, add_small=True, fix_borders=True):
    # city = os.path.splitext(fn)[0][5:]
    img_copy, ske = make_skeleton(root, debug, fix_borders)
    # print(np.where(ske>0))
    # cv2.imwrite('skeleton.png', ske)
    if ske is None:
        print('skeleton result is None')
    G = sknw.build_sknw(ske, multi=True)
    remove_small_terminal(G)
    node_lines = graph2lines(G)
    if not node_lines:
        print('graph2lines result is None')
    node = G.node
    deg = G.degree()
    wkt = []
    terminal_points = [i for i, d in deg.items() if d == 1]
