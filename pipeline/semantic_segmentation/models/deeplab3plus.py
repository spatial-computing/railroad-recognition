    ...
    def call(self, inputs):
        if self.upsampling:
            return K.tf.image.resize_bilinear(inputs, (inputs.shape[1] * self.upsampling[0],
                                              ...
                                              align_corners=True)
            #return tf.keras.backend.resize_images(inputs, inputs.shape[1] * self.upsampling[0],\
            #                                     inputs.shape[2] * self.upsampling[1], interpolation='bilinear')
        else:
            return K.tf.image.resize_bilinear(inputs, (self.output_size[0],
                                                       self.output_size[1]),
                                              align_corners=True)
            #return tf.keras.backend.resize_images(inputs, self.output_size[0],\
            #                                     self.output_size[1], interpolation='bilinear')
